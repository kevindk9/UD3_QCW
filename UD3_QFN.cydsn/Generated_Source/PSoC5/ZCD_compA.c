...
uint8 ZCD_compA_initVar = 0u;

/* Internal functions definitoin */
static void ZCD_compA_trimAdjust(uint8 nibble) ;

/* static ZCD_compA_backupStruct  ZCD_compA_backup; */
#if (CY_PSOC5A)
    static ZCD_compA_LOWPOWER_BACKUP_STRUCT  ZCD_compA_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 ZCD_compA_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: ZCD_compA_Init
********************************************************************************
*
* Summary:
...
void ZCD_compA_Init(void) 
{
    ...
    ZCD_compA_SetSpeed(ZCD_compA_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( ZCD_compA_DEFAULT_HYSTERESIS == 0u )
        ZCD_compA_CR |= ZCD_compA_HYST_OFF;
    #else
        ZCD_compA_CR &= (uint8)(~ZCD_compA_HYST_OFF);
    #endif /* ZCD_compA_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( ZCD_compA_DEFAULT_PWRDWN_OVRD == 0u )
            ZCD_compA_CR &= (uint8)(~ZCD_compA_PWRDWN_OVRD);
        #else 
            ZCD_compA_CR |= ZCD_compA_PWRDWN_OVRD;
        #endif /* ZCD_compA_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    ZCD_compA_CR |= ZCD_compA_MX_AO;

    /* Set default sync */
    ZCD_compA_CLK &= (uint8)(~ZCD_compA_SYNCCLK_MASK);
    #if ( ZCD_compA_DEFAULT_BYPASS_SYNC == 0u )
        ZCD_compA_CLK |= ZCD_compA_SYNC_CLK_EN;
    #else
        ZCD_compA_CLK |= ZCD_compA_BYPASS_SYNC;
    #endif /* ZCD_compA_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: ZCD_compA_Enable
********************************************************************************
*
* Summary:
...
{
    ZCD_compA_PWRMGR |= ZCD_compA_ACT_PWR_EN;
    ZCD_compA_STBY_PWRMGR |= ZCD_compA_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(ZCD_compA_restoreReg == 1u)
        {
            ZCD_compA_CR = ZCD_compA_lowPowerBackup.compCRReg;

            /* Clear the flag */
            ZCD_compA_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  ZCD_compA_initVar: Is modified when this function is called for the 
...
void ZCD_compA_Start(void) 
{
    ...
    if ( ZCD_compA_initVar == 0u )
    {
        ZCD_compA_Init();
        
        ZCD_compA_initVar = 1u;
    }   
...
void ZCD_compA_Stop(void) 
{
    ...
    ZCD_compA_PWRMGR &= (uint8)(~ZCD_compA_ACT_PWR_EN);
    ZCD_compA_STBY_PWRMGR &= (uint8)(~ZCD_compA_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        ZCD_compA_restoreReg = 1u;

        /* Save the control register before clearing it */
        ZCD_compA_lowPowerBackup.compCRReg = ZCD_compA_CR;
        ZCD_compA_CR = ZCD_compA_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: ZCD_compA_SetSpeed
********************************************************************************
*
* Summary:
...
void ZCD_compA_SetSpeed(uint8 speed) 
{
    ...
    ZCD_compA_CR = (ZCD_compA_CR & (uint8)(~ZCD_compA_PWR_MODE_MASK)) |
                           (speed & ZCD_compA_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == ZCD_compA_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            ZCD_compA_TR = ZCD_compA_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            ZCD_compA_TR0 = ZCD_compA_HS_TRIM_TR0;
            ZCD_compA_TR1 = ZCD_compA_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            ZCD_compA_TR = ZCD_compA_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            ZCD_compA_TR0 = ZCD_compA_LS_TRIM_TR0;
            ZCD_compA_TR1 = ZCD_compA_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: ZCD_compA_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( ZCD_compA_WRK & ZCD_compA_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: ZCD_compA_trimAdjust
********************************************************************************
*
* Summary:
...
static void ZCD_compA_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = ZCD_compA_WRK & ZCD_compA_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                ZCD_compA_TR |= ZCD_compA_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                ZCD_compA_TR0 |= ZCD_compA_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                ZCD_compA_TR |= ZCD_compA_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                ZCD_compA_TR1 |= ZCD_compA_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((ZCD_compA_WRK & ZCD_compA_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: ZCD_compA_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 ZCD_compA_ZeroCal(void) 
{
    ...
    tmpSW0 = ZCD_compA_SW0;
    tmpSW2 = ZCD_compA_SW2;
    tmpSW3 = ZCD_compA_SW3;

     /* Clear routing for inP, retain routing for inN */
    ZCD_compA_SW0 = 0x00u;
    ZCD_compA_SW2 = 0x00u;
    ZCD_compA_SW3 = tmpSW3 & (uint8)(~ZCD_compA_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = ZCD_compA_CR;
    ZCD_compA_CR |= (ZCD_compA_CAL_ON | ZCD_compA_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        ZCD_compA_TR = ZCD_compA_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        ZCD_compA_TR0 = ZCD_compA_DEFAULT_CMP_TRIM;
        ZCD_compA_TR1 = ZCD_compA_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (ZCD_compA_CR & ZCD_compA_PWR_MODE_MASK) == ZCD_compA_PWR_MODE_FAST)
    {
        ZCD_compA_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        ZCD_compA_trimAdjust(1u);
		ZCD_compA_trimAdjust(0u);
    }
    ...
    ZCD_compA_SW0 = tmpSW0;
    ZCD_compA_SW2 = tmpSW2;
    ZCD_compA_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) ZCD_compA_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)ZCD_compA_TR1 << 8u) | (ZCD_compA_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: ZCD_compA_LoadTrim
********************************************************************************
*
* Summary:
...
void ZCD_compA_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: ZCD_compA_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: ZCD_compA_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
