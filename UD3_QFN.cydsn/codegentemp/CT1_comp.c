...
uint8 CT1_comp_initVar = 0u;

/* Internal functions definitoin */
static void CT1_comp_trimAdjust(uint8 nibble) ;

/* static CT1_comp_backupStruct  CT1_comp_backup; */
#if (CY_PSOC5A)
    static CT1_comp_LOWPOWER_BACKUP_STRUCT  CT1_comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 CT1_comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: CT1_comp_Init
********************************************************************************
*
* Summary:
...
void CT1_comp_Init(void) 
{
    ...
    CT1_comp_SetSpeed(CT1_comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( CT1_comp_DEFAULT_HYSTERESIS == 0u )
        CT1_comp_CR |= CT1_comp_HYST_OFF;
    #else
        CT1_comp_CR &= (uint8)(~CT1_comp_HYST_OFF);
    #endif /* CT1_comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( CT1_comp_DEFAULT_PWRDWN_OVRD == 0u )
            CT1_comp_CR &= (uint8)(~CT1_comp_PWRDWN_OVRD);
        #else 
            CT1_comp_CR |= CT1_comp_PWRDWN_OVRD;
        #endif /* CT1_comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    CT1_comp_CR |= CT1_comp_MX_AO;

    /* Set default sync */
    CT1_comp_CLK &= (uint8)(~CT1_comp_SYNCCLK_MASK);
    #if ( CT1_comp_DEFAULT_BYPASS_SYNC == 0u )
        CT1_comp_CLK |= CT1_comp_SYNC_CLK_EN;
    #else
        CT1_comp_CLK |= CT1_comp_BYPASS_SYNC;
    #endif /* CT1_comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: CT1_comp_Enable
********************************************************************************
*
* Summary:
...
{
    CT1_comp_PWRMGR |= CT1_comp_ACT_PWR_EN;
    CT1_comp_STBY_PWRMGR |= CT1_comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(CT1_comp_restoreReg == 1u)
        {
            CT1_comp_CR = CT1_comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            CT1_comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  CT1_comp_initVar: Is modified when this function is called for the 
...
void CT1_comp_Stop(void) 
{
    ...
    CT1_comp_PWRMGR &= (uint8)(~CT1_comp_ACT_PWR_EN);
    CT1_comp_STBY_PWRMGR &= (uint8)(~CT1_comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        CT1_comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        CT1_comp_lowPowerBackup.compCRReg = CT1_comp_CR;
        CT1_comp_CR = CT1_comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: CT1_comp_SetSpeed
********************************************************************************
*
* Summary:
...
void CT1_comp_SetSpeed(uint8 speed) 
{
    ...
    CT1_comp_CR = (CT1_comp_CR & (uint8)(~CT1_comp_PWR_MODE_MASK)) |
                           (speed & CT1_comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == CT1_comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            CT1_comp_TR = CT1_comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CT1_comp_TR0 = CT1_comp_HS_TRIM_TR0;
            CT1_comp_TR1 = CT1_comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            CT1_comp_TR = CT1_comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CT1_comp_TR0 = CT1_comp_LS_TRIM_TR0;
            CT1_comp_TR1 = CT1_comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: CT1_comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( CT1_comp_WRK & CT1_comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: CT1_comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void CT1_comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = CT1_comp_WRK & CT1_comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CT1_comp_TR |= CT1_comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CT1_comp_TR0 |= CT1_comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CT1_comp_TR |= CT1_comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CT1_comp_TR1 |= CT1_comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((CT1_comp_WRK & CT1_comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: CT1_comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 CT1_comp_ZeroCal(void) 
{
    ...
    CT1_comp_SW0 = 0x00u;
    CT1_comp_SW2 = 0x00u;
    CT1_comp_SW3 = tmpSW3 & (uint8)(~CT1_comp_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = CT1_comp_CR;
    CT1_comp_CR |= (CT1_comp_CAL_ON | CT1_comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        CT1_comp_TR = CT1_comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        CT1_comp_TR0 = CT1_comp_DEFAULT_CMP_TRIM;
        CT1_comp_TR1 = CT1_comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (CT1_comp_CR & CT1_comp_PWR_MODE_MASK) == CT1_comp_PWR_MODE_FAST)
    {
        CT1_comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        CT1_comp_trimAdjust(1u);
		CT1_comp_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) CT1_comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)CT1_comp_TR1 << 8u) | (CT1_comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: CT1_comp_LoadTrim
********************************************************************************
*
* Summary:
...
void CT1_comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: CT1_comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: CT1_comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
