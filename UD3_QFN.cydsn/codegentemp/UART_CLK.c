...
void UART_CLK_Start(void) 
{
    ...
    UART_CLK_CLKEN |= UART_CLK_CLKEN_MASK;
	UART_CLK_CLKSTBY |= UART_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: UART_CLK_Stop
********************************************************************************
*
* Summary:
...
void UART_CLK_Stop(void) 
{
    ...
    UART_CLK_CLKEN &= (uint8)(~UART_CLK_CLKEN_MASK);
	UART_CLK_CLKSTBY &= (uint8)(~UART_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: UART_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((UART_CLK_CLKEN & UART_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = UART_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = UART_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* UART_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(UART_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(UART_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: UART_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = UART_CLK_GetSourceRegister();
    uint16 oldDivider = UART_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = UART_CLK_CLKEN & UART_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((UART_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: UART_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(UART_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: UART_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    UART_CLK_MOD_SRC |= modeBitMask & (uint8)UART_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: UART_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    UART_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(UART_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: UART_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return UART_CLK_MOD_SRC & (uint8)(UART_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: UART_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = UART_CLK_GetDividerRegister();
    uint8 oldSrc = UART_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        UART_CLK_MOD_SRC |= CYCLK_SSS;
        UART_CLK_MOD_SRC =
            (UART_CLK_MOD_SRC & (uint8)(~UART_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: UART_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return UART_CLK_MOD_SRC & UART_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: UART_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    UART_CLK_PHASE = clkPhase & UART_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: UART_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return UART_CLK_PHASE & UART_CLK_PHASE_MASK;
}
