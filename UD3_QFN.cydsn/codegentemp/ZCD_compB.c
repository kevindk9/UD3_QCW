...
uint8 ZCD_compB_initVar = 0u;

/* Internal functions definitoin */
static void ZCD_compB_trimAdjust(uint8 nibble) ;

/* static ZCD_compB_backupStruct  ZCD_compB_backup; */
#if (CY_PSOC5A)
    static ZCD_compB_LOWPOWER_BACKUP_STRUCT  ZCD_compB_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 ZCD_compB_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: ZCD_compB_Init
********************************************************************************
*
* Summary:
...
void ZCD_compB_Init(void) 
{
    ...
    ZCD_compB_SetSpeed(ZCD_compB_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( ZCD_compB_DEFAULT_HYSTERESIS == 0u )
        ZCD_compB_CR |= ZCD_compB_HYST_OFF;
    #else
        ZCD_compB_CR &= (uint8)(~ZCD_compB_HYST_OFF);
    #endif /* ZCD_compB_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( ZCD_compB_DEFAULT_PWRDWN_OVRD == 0u )
            ZCD_compB_CR &= (uint8)(~ZCD_compB_PWRDWN_OVRD);
        #else 
            ZCD_compB_CR |= ZCD_compB_PWRDWN_OVRD;
        #endif /* ZCD_compB_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    ZCD_compB_CR |= ZCD_compB_MX_AO;

    /* Set default sync */
    ZCD_compB_CLK &= (uint8)(~ZCD_compB_SYNCCLK_MASK);
    #if ( ZCD_compB_DEFAULT_BYPASS_SYNC == 0u )
        ZCD_compB_CLK |= ZCD_compB_SYNC_CLK_EN;
    #else
        ZCD_compB_CLK |= ZCD_compB_BYPASS_SYNC;
    #endif /* ZCD_compB_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: ZCD_compB_Enable
********************************************************************************
*
* Summary:
...
{
    ZCD_compB_PWRMGR |= ZCD_compB_ACT_PWR_EN;
    ZCD_compB_STBY_PWRMGR |= ZCD_compB_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(ZCD_compB_restoreReg == 1u)
        {
            ZCD_compB_CR = ZCD_compB_lowPowerBackup.compCRReg;

            /* Clear the flag */
            ZCD_compB_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  ZCD_compB_initVar: Is modified when this function is called for the 
...
void ZCD_compB_Start(void) 
{
    ...
    if ( ZCD_compB_initVar == 0u )
    {
        ZCD_compB_Init();
        
        ZCD_compB_initVar = 1u;
    }   
...
void ZCD_compB_Stop(void) 
{
    ...
    ZCD_compB_PWRMGR &= (uint8)(~ZCD_compB_ACT_PWR_EN);
    ZCD_compB_STBY_PWRMGR &= (uint8)(~ZCD_compB_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        ZCD_compB_restoreReg = 1u;

        /* Save the control register before clearing it */
        ZCD_compB_lowPowerBackup.compCRReg = ZCD_compB_CR;
        ZCD_compB_CR = ZCD_compB_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: ZCD_compB_SetSpeed
********************************************************************************
*
* Summary:
...
void ZCD_compB_SetSpeed(uint8 speed) 
{
    ...
    ZCD_compB_CR = (ZCD_compB_CR & (uint8)(~ZCD_compB_PWR_MODE_MASK)) |
                           (speed & ZCD_compB_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == ZCD_compB_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            ZCD_compB_TR = ZCD_compB_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            ZCD_compB_TR0 = ZCD_compB_HS_TRIM_TR0;
            ZCD_compB_TR1 = ZCD_compB_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            ZCD_compB_TR = ZCD_compB_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            ZCD_compB_TR0 = ZCD_compB_LS_TRIM_TR0;
            ZCD_compB_TR1 = ZCD_compB_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: ZCD_compB_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( ZCD_compB_WRK & ZCD_compB_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: ZCD_compB_trimAdjust
********************************************************************************
*
* Summary:
...
static void ZCD_compB_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = ZCD_compB_WRK & ZCD_compB_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                ZCD_compB_TR |= ZCD_compB_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                ZCD_compB_TR0 |= ZCD_compB_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                ZCD_compB_TR |= ZCD_compB_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                ZCD_compB_TR1 |= ZCD_compB_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((ZCD_compB_WRK & ZCD_compB_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: ZCD_compB_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 ZCD_compB_ZeroCal(void) 
{
    ...
    tmpSW0 = ZCD_compB_SW0;
    tmpSW2 = ZCD_compB_SW2;
    tmpSW3 = ZCD_compB_SW3;

     /* Clear routing for inP, retain routing for inN */
    ZCD_compB_SW0 = 0x00u;
    ZCD_compB_SW2 = 0x00u;
    ZCD_compB_SW3 = tmpSW3 & (uint8)(~ZCD_compB_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = ZCD_compB_CR;
    ZCD_compB_CR |= (ZCD_compB_CAL_ON | ZCD_compB_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        ZCD_compB_TR = ZCD_compB_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        ZCD_compB_TR0 = ZCD_compB_DEFAULT_CMP_TRIM;
        ZCD_compB_TR1 = ZCD_compB_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (ZCD_compB_CR & ZCD_compB_PWR_MODE_MASK) == ZCD_compB_PWR_MODE_FAST)
    {
        ZCD_compB_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        ZCD_compB_trimAdjust(1u);
		ZCD_compB_trimAdjust(0u);
    }
    ...
    ZCD_compB_SW0 = tmpSW0;
    ZCD_compB_SW2 = tmpSW2;
    ZCD_compB_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) ZCD_compB_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)ZCD_compB_TR1 << 8u) | (ZCD_compB_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: ZCD_compB_LoadTrim
********************************************************************************
*
* Summary:
...
void ZCD_compB_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: ZCD_compB_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: ZCD_compB_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
