...
/*******************************************************************************
* Function Name: Cysamp_clk_Start
********************************************************************************
*
* Summary:
...
void Cysamp_clk_Start(void) 
{
    ...
    Cysamp_clk_CLKEN |= Cysamp_clk_CLKEN_MASK;
	Cysamp_clk_CLKSTBY |= Cysamp_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Cysamp_clk_Stop
********************************************************************************
*
* Summary:
...
void Cysamp_clk_Stop(void) 
{
    ...
    Cysamp_clk_CLKEN &= (uint8)(~Cysamp_clk_CLKEN_MASK);
	Cysamp_clk_CLKSTBY &= (uint8)(~Cysamp_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Cysamp_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Cysamp_clk_CLKEN & Cysamp_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Cysamp_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Cysamp_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Cysamp_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Cysamp_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Cysamp_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Cysamp_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Cysamp_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Cysamp_clk_GetSourceRegister();
    uint16 oldDivider = Cysamp_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Cysamp_clk_CLKEN & Cysamp_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Cysamp_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Cysamp_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Cysamp_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Cysamp_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Cysamp_clk_MOD_SRC |= modeBitMask & (uint8)Cysamp_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Cysamp_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Cysamp_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Cysamp_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Cysamp_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Cysamp_clk_MOD_SRC & (uint8)(Cysamp_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Cysamp_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Cysamp_clk_GetDividerRegister();
    uint8 oldSrc = Cysamp_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Cysamp_clk_MOD_SRC |= CYCLK_SSS;
        Cysamp_clk_MOD_SRC =
            (Cysamp_clk_MOD_SRC & (uint8)(~Cysamp_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Cysamp_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Cysamp_clk_MOD_SRC & Cysamp_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Cysamp_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Cysamp_clk_PHASE = clkPhase & Cysamp_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Cysamp_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Cysamp_clk_PHASE & Cysamp_clk_PHASE_MASK;
}
